...
static char *read_fixture_file(const char *path, long *out_len) {
  ...
  buf[read_len] = '\0';
  if (out_len)
    *out_len = (long)read_len;
  fclose(f);
  return buf;
}

void test_file_io_setup_and_read_fixture(void) {
  // Try to read tests/fixtures/test.css
  const char *path = "tests/fixtures/test.css";

  FILE *f = fopen(path, "rb");
  TEST_ASSERT_NOT_NULL_MESSAGE(f, "test.css fixture not found");
...
void test_manual_minified(void) {
  const char *css =
      ".d-flex{display:flex!important}.flex-column{flex-direction:column!"
      "important}.h-100{height:100%!important}.bg-turquoise{background-color:"
      "turquoise!important}.unused{color:red}";
...
void test_manual_minified_media_queries(void) {
  const char *css =
      ".d-flex{display:flex!important}.flex-column{flex-direction:column!"
      "important}.h-100{height:100%!important}.bg-turquoise{background-color:"
      "turquoise!important}.unused{color:red}@media{.mediatest{color:red}}";
  const char *used[] = {"d-flex", "flex-column", "h-100", "bg-turquoise",
                        "mediatest"};
...
void test_bootstrap_reduction(void) {
  long len;
  char *css = read_fixture_file("tests/fixtures/bootstrap.css", &len);
  ...
  OptimizerConfig config = {.used_classes = used,
                            ...
                            .attr_count = 0,
                            .mode = LXB_CSS_OPTIM_MODE_STRICT};
  char *result = css_optimize(css, (size_t)len, &config);
  TEST_ASSERT_NOT_NULL(result);
  ...
  TEST_ASSERT_NULL_MESSAGE(
      strstr(result, ".accordion-button"),
      "Bootstrap accordion unused class should be removed");
  TEST_ASSERT_NULL_MESSAGE(
      strstr(result, ".btn-primary"),
      "Bootstrap btn-primary unused class should be removed");
...
void test_html_tag_removal(void) {
  ...
  long h_len;
  char *html_content =
      read_fixture_file("tests/fixtures/htmlrmtest.html", &h_len);
  ...
  long c_len;
  char *css_content =
      read_fixture_file("tests/fixtures/htmlrmtest.css", &c_len);
  ...
  OptimizerConfig config = {.used_classes = (const char **)used_classes.items,
                            ...
                            .attr_count = 0,
                            .mode = LXB_CSS_OPTIM_MODE_STRICT};
  char *result = css_optimize(css_content, (size_t)c_len, &config);
...
void test_attr_and_pseudo(void) {
  ...
  // 2. Read attrtest.css
  long c_len;
  char *css = read_fixture_file("tests/fixtures/attrtest.css", &c_len);
...
void run_integration_tests(void) {
  RUN_TEST(test_file_io_setup_and_read_fixture);
