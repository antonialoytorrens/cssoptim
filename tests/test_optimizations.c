...
void test_remove_unused_keyframes(void) {
  const char *css =
      "@keyframes used { from { opacity: 0; } } @keyframes unused { from { "
      "opacity: 1; } } .class { animation: used 1s; }";
  const char *used_classes[] = {"class"};

  OptimizerConfig config = {.used_classes = used_classes,
                            .class_count = 1,
                            .used_tags = NULL,
                            .tag_count = 0,
                            .used_attrs = NULL,
...
void test_remove_form_pseudoelements_without_forms(void) {
  const char *css =
      ".test { color: red } ::file-selector-button { font: inherit } "
